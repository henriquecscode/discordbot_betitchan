...
exports.MusicPlay = function (message, link){
    ...
    server.queue.push(link); //Adds the link to the queuekk
...
function MusicQueue(connection){
    ...
    server.dispatcher = connection.playStream(YTDL(server.queue[0], {filter: "audioonly"})); //Stream the music, using YTDL to download the queue[0] link, though only its audio (filter: "audioonly")
